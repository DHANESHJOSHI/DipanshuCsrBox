...
export default function LoginPage() {
  ...
  const onLogin = async (user) => {
    try {
      ...
      toast.error("Login failed: " + (error.response?.data?.message || error.message || "Unknown Error"));
    } finally {
      setLoading(false);
    }
  };
  ...
  return loading ? (
    <div role="status" className="flex flex-col items-center justify-center space-y-4">
      ...
      <div className="typewriter bg-white">
        <p className="font-bold text-blue-700">Please wait while configuring your folders and fetching your team details</p>
      </div>
    </div>
  ) : (
    <>
